...
### Split-N device pipeline for B1 M33 N513 K256
...
The split raises global Cube utilization from 4.95% to 22.34%, but the first
stage still takes almost as long as the single-core kernel and the 5.680 us
finalizer raises total device time to 40.320 us. This profiler evidence is why
the final policy does not select key 130 at this workload size.
...
3. **Inter-core pipeline:** skipped; the kernel has atomic accumulation but no
   explicit inter-core producer/consumer synchronization.
...
2. Implemented: hoist invariant `SetOrgShape` and bias state out of the
   per-task loop.
3. Implemented: key 130 writes per-row partial maxima for N shards, then a
   Vector kernel reduces shards and sums M. The final conservative gate is
